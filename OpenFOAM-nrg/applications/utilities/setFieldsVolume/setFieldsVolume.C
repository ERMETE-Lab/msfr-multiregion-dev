...
int main(int argc, char *argv[])
{
    ...
    forAll(regions, regionI)
    {
        ...
        if (source().setType() == topoSetSource::CELLSETSOURCE)
        {
            ...
            // Get cellZone volume
            word zoneName(region.dict().lookup("name"));
            const label cellZoneID = mesh.cellZones().findZoneID(zoneName);
            ...
            forAll(cellsZone, cI)
            {
            	cellZoneVolume += mesh.V()[cellsZone[cI]];
            }
            
            Info<< "	Calculated zone " << zoneName << " total volume is " << cellZoneVolume << "." << endl;
            
            // Get and update dictionary entry
            ITstream& fieldEntry = region.dict().lookup("fieldValues");                        
            scalar oldValue = fieldEntry[3].number();
            scalar newValue = oldValue/cellZoneVolume;
            fieldEntry[3] = token(newValue);
